...
pub async fn imu(i2c: I2c<'static, Bno055I2cMode>) {
    ...
    loop {
        ...
        let send_imu_message = |imu: &mut Bno055<I2c<'_, mode::Blocking>>| -> Option<()> {
            defmt::info!("{:?}", ImuMessage {
                quaternion: imu.quaternion().ok()?.into(),
                linear_acceleration: imu.linear_acceleration().ok()?.into(),
                gravity: imu.gravity().ok()?.into(),
                euler_angles: imu.euler_angles().ok()?.into(),
                acceleration: imu.accel_data().ok()?.into(),
                gyro: imu.gyro_data().ok()?.into(),
                mag: imu.mag_data().ok()?.into(),
                temperature: imu.temperature().ok()? as f32,
                timestamp: Instant::now().as_micros(),
            });
            Some(())
        };

        send_imu_message(&mut imu).unwrap_or_else(|| {
            defmt::error!("Failed to send IMU message");
        });
...
pub async fn gps(mut uart: Uart<'static, mode::Async>) {
    ...
    let send_gps_message = |nmea: &Nmea| -> Option<()> {
        defmt::info!("{:?}", GpsMessage {
            fix_time: nmea.fix_time?,
            fix_type: nmea.fix_type?,
            latitude: nmea.latitude?,
            longitude: nmea.longitude?,
            altitude: nmea.altitude?,
            num_of_fix_satellites: nmea.num_of_fix_satellites? as u8,
            timestamp: Instant::now().as_micros(),
        });
        Some(())
    };
    ...
    loop {
        ...
        match nmea.parse(message) {
            Ok(_) => {
                ...
                send_gps_message(&nmea).unwrap_or_else(|| {
                    defmt::error!("Failed to send GPS message");
                });
            }
            Err(e) => {
