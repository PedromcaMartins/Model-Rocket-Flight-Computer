...
> (
    mut gps: S,
    status_sender: Sender<'static, M, Result<GpsSystemStatus, usize>, DEPTH_STATUS>,
        ...
        match select (
            ...
            Either::First(()) => {
                match gps.parse_new_message().await {
                    Ok(msg) => {
                        send_to_system_status!(status_sender, error_sending_status, GpsSystemStatus::MessageParsed);

                        if postcard_sender.publish::<GpsTopic>(VarSeq::Seq4(seq.0), &msg).await.is_ok() {
                            seq += 1;
                        } else { 
                            send_to_system_status!(status_sender, error_sending_status, GpsSystemStatus::FailedToPublishToPostcard);
                        }

                        if sd_card_sender.try_send(msg).is_err() {
                            send_to_system_status!(status_sender, error_sending_status, GpsSystemStatus::FailedToPublishToSdCard);
                        }
                    }, 
                    Err(_) => send_to_system_status!(status_sender, error_sending_status, GpsSystemStatus::FailedToParseMessage),
                }
